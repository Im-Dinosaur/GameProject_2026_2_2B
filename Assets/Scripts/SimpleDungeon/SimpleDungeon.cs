...
public class SimpleDungeon : MonoBehaviour
{
    ...
    private readonly Dictionary<Vector2Int, Room> rooms = new();  //방의 중심 좌표를 key로 사용하여 각 room 정보를 저장
    private readonly HashSet<Vector2Int> floors = new();          //방과 복도를 포함한 모단 바닥 좌표를 중복 없이 저장
    private readonly HashSet<Vector2Int> roomFloors = new();      //복도를 제외한 방 자체의 바닥 좌표만 저장
    ...
    private void AssignSpecialRooms()
    {
        ...
        int treasureCount = Mathf.Min(Mathf.Max(1, rooms.Count / 4), normalRooms.Count);
    ...
    private void CreateRoomsAndCorridors()
    {
        ...
        TryAddRoom(Vector2Int.zero, startSize, RoomType.Start);  //던전의 원점에 시작 방을 생성
        ...
        {
            //현재 생성되어있는 방들을 List로 복사
            List<Room> existingRooms = new(rooms.Values);

            Room parent = existingRooms[Random.Range(0, existingRooms.Count)];

            List<Vector2Int> directions = ShuffledDirections();  //상하좌우 방향의 검사순서를 랜덤하게 섞음

            foreach(Vector2Int direction in directions) //랜덤하게 섞인 네 방향을 하나씩 검사
            {
                Vector2Int center = parent.center + direction * roomSpacing;  //부모방 중심에서 해당 방향으로 roomSpacing만큼 떨어진 새 중심점을 계산
    ...
    {
        //원번 방향 배열을 수정하지 않도록 새로운 List만든다.
        List<Vector2Int> result = new(Directions);

        for(int i = result.Count - 1; i > 0; i--)  //뒤쪽 원소부터 하나씩 랜덤위치와 교환 하는 Fisher-Yates 방식
        {
    ...
    private bool TryAddRoom(Vector2Int center, int size, RoomType type)
    {
        ...
        //기본 방 주변 한칸의 여백까지 검사하여 방끼리 바로 붙지 않게 함.
        for(int x = min - 1; x <= max; x++)  //X 범위보다 한칸
        {
            for(int y = min -1; y <= max; y++)  //Y 범위보다 한칸
            {
                if (roomFloors.Contains(center + new Vector2Int(x, y))) return false; //검사하려는 좌표가 기본 방 바닥에 포함되어 있으면 방 생성을 실패
            }
        }

        Room room = new Room(center, size, type);
        rooms.Add(center, room);  //방의 중심 좌표를 Key로 사용하여 Dictionary에 방을 추가함

        for(int x = min; x <= max; x++)
        {
            for(int y = min; y <= max; y++)
            {
                Vector2Int cell = center + new Vector2Int(x, y); //방 중심에 상대 좌표를 더하여 실제 셀 좌표를 계산
    ...
    private void CreateCorridor(Vector2Int start, Vector2Int end)  //시작 방과 끝방 사이에 ㄱ자 형태의 복도를 생성
    {
        Vector2Int current = start;

        if(Random.value < 0.5f)  //50% 확률로 X축과 Y축 중 어느 방향을 먼저 이동할지 결정
        {
    ...
    {
        rooms.Clear(); //저장되어있던 모든 방 정보를 삭제함
        floors.Clear();//저장되어있던 모든 바닥과 복도 좌표를 삭제함
        roomFloors.Clear();//저장되어있던 방 전용 바닥 좌표를 삭제함
        ...
        else DestroyImmediate(oldRoot.gameObject);  //에디터 모드라면 즉시 삭제 할 수 있는 DestroyImmediate를 사용함
    }
